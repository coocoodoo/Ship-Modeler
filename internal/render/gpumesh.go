...
type BodyGPU struct {
	...
	// vertFace is the source face of each emitted corner.
	vertFace []int
...
func BuildBodyGPU(m *mesh.Mesh) *BodyGPU {
	...
	tris := m.Triangulate()
	// AO is evaluated per pixel; no extra triangles are needed for shading.
	n := len(tris) * 3
	...
	for _, t := range tris {
		...
		emit := func(p geom.Vec3) {
			...
			// Red and green retain the pick key; shading does not use it.
