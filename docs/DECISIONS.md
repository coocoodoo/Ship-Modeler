...
**V-154 · The icons are redrawn as strokes, and the rasterizer is rebuilt to
draw them.** "The current icons feel low quality" — and they did. The set
from V-147 was authored as filled silhouettes because that is all the engine
could draw, and it showed: several icons rasterized to indistinguishable
blobs, weights wandered between hairline and heavy, and shapes that needed a
hole depended on the even-odd rule to punch one.

The cause was the engine, not the drawing. Every modern icon set is drawn as
centre-lines with one weight, and a fills-only renderer forces an author to
hand-build each glyph as an outline — which is exactly how a set comes out
uneven, because the consistency then depends on whoever is typing the
numbers rather than on the format. So the engine learned to stroke: a path
with `stroke` and `stroke-width` is stamped as a quad per segment and a disc
per joint and round cap, all wound the same way, so a nonzero fill of the
pile is exactly their union. It needs no mitre arithmetic and cannot throw
the spike a naive offset gives a sharp corner.

Two other gaps closed with it. Winding is now real — nonzero by default,
even-odd when the file asks — where before everything was even-odd. And
antialiasing is computed rather than sampled: each of sixteen sub-scanlines
contributes the *exact* fraction of every pixel its spans cover, so an edge
at any angle lands on a smooth ramp instead of the sixteen steps a 4×4
supersample can express. At 18 px, where every icon in this program actually
lives, that is the whole difference between a crisp glyph and a ragged one.

All 53 icons were then redrawn on one system: a 24×24 grid, 2-unit strokes
with round caps and joins, about 2 units of margin, and no detail finer than
2 units so every glyph survives 18 px. Filled marks are used only where a
stroke cannot say it — a pupil, a handle knob, a solid arrowhead.

Reviewing them at size changed several. The gradient read as a paragraph of
text (three bars became a filled ramp wedge); settings read as a rifle scope
(a gear turns to mush at this size, so it is two sliders); the eyedropper was
indistinguishable from the pencil it sits beside in the same row, so it kept
its bulb; the point tool and the mid-line were both a cross, so the mid-line
took end knobs and a shorter tick; and Move's arrowheads were too small to
register in the toolbar, which left it reading as another cross. `IconMask`
and `IconNames` are exported so a test can write contact sheets of the whole
set without a GPU, which is how all of that was caught — the rasterizer
produces an image, and only the upload needs a context.
